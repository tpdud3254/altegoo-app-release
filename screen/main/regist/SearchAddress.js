  ...
  const Help = ({ number, text }) => (
    <HelpWrapper>
      <MaterialCommunityIcons
        ...
        color="#777777"
      />
      <PlainText style={{ marginLeft: 5, color: "#777777" }}>{text}</PlainText>
    </HelpWrapper>
  );
    ...
    <MainLayout>
      <Container behavior="position" keyboardVerticalOffset={0}>
        <InputAddress>
          <Help
            number="1"
            text={`'${getHelpText(registInfo.upDown)}' 주소 검색하기`}
          />
          <InputAddWrapper>
            <Search onPress={() => searchAddress(0)}>
              {route?.params?.selectAddress1?.address &&
              route?.params?.selectAddress1?.address !== "" ? (
                <PlainText>{route?.params?.selectAddress1?.address}</PlainText>
              ) : (
            ...
            <DetailAddress
              ...
              onChangeText={(text) => setValue("detailAddress1", text)}
            />
          </InputAddWrapper>
        </InputAddress>
        ...
        {registInfo.upDown === "양사" ? (
          <InputOtherAddress>
            <Help
              number="2"
              text={`'${
                getHelpText(registInfo.upDown) === "내림" ? "올림" : "내림"
              }' 주소 검색하기`}
            />
            <InputAddWrapper>
              ...
              <DetailAddress
                placeholder="상세주소 입력하기"
                onChangeText={(text) => setValue("detailAddress2", text)}
              />
            </InputAddWrapper>
          </InputOtherAddress>
        ) : null}
