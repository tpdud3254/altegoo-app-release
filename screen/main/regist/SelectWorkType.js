...
function SelectWorkType({ navigation }) {
  ...
  const onNextStep = () => {
    ...
    setRegistInfo({
      ...
      upDown: upDown === "up" ? "올림" : upDown === "down" ? "내림" : "양사",
      bothType: both,
    });
  ...
  const Help = ({ number, cur, text }) => (
    <HelpContainer>
      ...
      />
      <PlainText style={{ marginLeft: 5, color: cur ? "black" : "#777777" }}>
        {text}
      </PlainText>
    </HelpContainer>
  );
      ...
      <Container>
        <VehicleContainer>
          ...
          >
            <VehicleWrapper>
              <Radio>
                <RadioButton value={1} color={color.sub.blue} />
                <PlainText>사다리</PlainText>
              </Radio>
              <Radio>
                <RadioButton value={2} color={color.sub.blue} />
        ...
        <TypeContainer>
          <Help number="2" cur={cur === 2} text="작업 유형을 선택해주세요." />
          <TypeWrapper>
            <TypeButton
              type="down"
        ...
        {upDown === "both" ? (
          <BothContainer>
            <Help number="3" cur={cur === 3} text="작업 순서를 선택해주세요." />
            <BothWrapper onPress={() => setBoth(1)} selected={both === 1}>
              <Both>
                <DownIcon />
            ...
            <BothWrapper onPress={() => setBoth(2)} selected={both === 2}>
              <Both>
                <UpIcon />
                <PlainText>올림</PlainText>
                <Feather
