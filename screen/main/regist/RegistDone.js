...
function RegistDone({ navigation }) {
  ...
  const getWorkFloor = () => {
    ...
    if (info.upDown === "양사") {
      text = `${info.floor}층(${info.bothType === 1 ? "내림" : "올림"}) > ${
        info.otherFloor
      }층(${info.bothType === 1 ? "올림" : "내림"})`;
    } else {
      text = `${info.floor}층`;
    }
    ...
    <MainLayout>
            ...
            <View>
              ...
              {registInfo.upDown === "양사" ? (
                <>
                  <Row
                    title={
                      (registInfo.bothType === 1 ? "내림" : "올림") + " 주소"
                    }
                    content={registInfo.address}
                  />
                  <Row
                    title={
                      (registInfo.bothType === 1 ? "올림" : "내림") + " 주소"
                    }
              ...
              <Row title="현장 연락처" content={registInfo.directPhone} />
              <Row
                title="작업 비용"
                content={numberWithComma(registInfo.price) + "AP"}
              />
              <Row
                title="적립 포인트"
                content={numberWithComma(registInfo.point) + "AP"}
              />
              <Row
                title="긴급 오더"
      ...
      <ButtonContainer>
        <Button onPress={onOrderModify}>
          <SubTitleText style={{ fontSize: 20 }}>작업수정</SubTitleText>
        </Button>
        <VerticalDivider color={theme.textBoxColor} />
        <Button onPress={onPay}>
          <SubTitleText style={{ fontSize: 20 }}>결제하기</SubTitleText>
        </Button>
      </ButtonContainer>
    </MainLayout>
  );
