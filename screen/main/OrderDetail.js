...
function OrderDetail({ route, navigation }) {
  ...
  const getWorkTime = () => {
    ...
    let text = `${workTime.getFullYear()}년 ${
      ...
      workTime.getDate() < 10 ? "0" + workTime.getDate() : workTime.getDate()
    }일 (${getDay(workTime.getDay())}) ${
      workTime.getHours() < 10 ? "0" + workTime.getHours() : workTime.getHours()
    }:${
  ...
  const getWorkFloor = () => {
    ...
    if (order.type === "양사") {
      text = `${order.floor}층(${order.bothType === 1 ? "내림" : "올림"}) > ${
        order.otherFloor
      }층(${order.bothType === 1 ? "올림" : "내림"})`;
    } else {
      text = `${order.floor}층`;
    }
          ...
          <View>
            <Container>
              ...
              {order.type === "양사" ? (
                <>
                  <Row
                    title={(order.bothType === 1 ? "내림" : "올림") + " 주소"}
                    content={order.address1 + " " + order.detailAddress1}
                  />
                  <Row title="" button={1} />
                  {showMap1 ? (
                    <View
                  ...
                  <Row
                    title={(order.bothType === 1 ? "올림" : "내림") + " 주소"}
                    content={order.address2 + " " + order.detailAddress2}
                  />
                  <Row title="" button={2} />
                  {showMap2 ? (
                    <View
              ...
              <Row title="특이 사항" content={order.memo || "없음"} />
            </Container>
